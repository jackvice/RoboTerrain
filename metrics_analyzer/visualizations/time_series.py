...
class TimeSeriesVisualizer:
    ...
    def plot_velocity_roughness_comparison(self,
        ...
        for i, (trial_name, df) in enumerate(trial_data.items()):
            ...
            # Plot velocity on top subplot - keep data access as 'Current Velocity' but update label
            ax1.plot(df['Steps'], df['Current Velocity'], 
                    label=f'{trial_name} VORT', 
                    color=color)
            
            # Plot IMU Acceleration Magnitude on bottom subplot
            ax2.plot(df['Steps'], df['IMU Acceleration Magnitude'], 
                    label=f'{trial_name} IMU Magnitude',
                    color=color)
        
        # Customize top subplot (Velocity)
        ax1.set_ylabel('Velocity Over Rough Terrain (m/s)')
...
class TimeSeriesVisualizer_origin:
    """Visualization class for time series metrics data."""
