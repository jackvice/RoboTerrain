...
def calculate_success_rate(log_file, start_time_str, window_minutes):
    # Convert input time to the correct timestamp format
    dt = datetime.strptime(start_time_str, "%Y-%m-%d %H:%M:%S")
    # Convert to timestamp using a different epoch base (based on your log timestamps)
    ...
    df = pd.read_csv(log_file, header=None, 
                    names=['timestamp', 'event', 'episode', 'position'])
