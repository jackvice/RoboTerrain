...
def attempt_load(weights, map_location=None):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        # ``weights_only=False`` is required for the legacy YOLOv7 checkpoint
        # that ships with this repository.  In PyTorch >= 2.6 the default
        # flipped to ``True`` which rejects pickled NumPy globals.
        try:
            ckpt = torch.load(w, map_location=map_location, weights_only=False)
        except TypeError:
            # PyTorch < 2.4 doesn't accept the ``weights_only`` kwarg.
