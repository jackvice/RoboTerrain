...
namespace vesc {
    ...
    vescChannelStatus BridgedVescArray::parseReceivedMessage(std::vector<uint8_t> robotmsg) 
    {
        ...
        // Default return if commandId doesn't match any known case
        return vescChannelStatus{.vescId = 0, .current = 0, .rpm = 0, .duty = 0, .voltage = 0, .current_in = 0, .dataValid = false};      
    }
