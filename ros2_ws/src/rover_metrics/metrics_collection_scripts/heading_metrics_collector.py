...
def quat_to_yaw_radians(w: float, x: float, y: float, z: float) -> float:
    """Convert quaternion to yaw angle in radians."""
    return math.atan2(2.0 * (w * z + x * y), 1.0 - 2.0 * (y * y + z * z))

def calculate_direction_degrees(history: deque) -> Optional[float]:
    """Calculate direction from position history (t-2 to t-0)."""
    if len(history) < 3:
        return None
    pos_old = history[0]  # t-2
    pos_new = history[2]  # t-0
    dx = pos_new[0] - pos_old[0]
    dy = pos_new[1] - pos_old[1]
    if abs(dx) < 0.01 and abs(dy) < 0.01:  # stationary
        return None
    return math.degrees(math.atan2(dy, dx))

def robot_heading_degrees(pose) -> float:
    """Convert robot pose quaternion to heading in degrees."""
    w = pose.orientation.w
    x = pose.orientation.x  
    y = pose.orientation.y
    z = pose.orientation.z
    yaw_rad = quat_to_yaw_radians(w, x, y, z)
...
def main() -> None:
    ...
    # --- config & file -------------------------------------------------------
    world_name = "inspect"
    csv_path = f"robot_metrics_head_{world_name}_{time.strftime('%m_%d_%H-%M')}.csv"
    ...
    def on_actor1_pose(msg: Pose) -> None:
        nonlocal actor1_xy, actor1_history
        pos = (float(msg.position.x), float(msg.position.y))
        actor1_xy = pos
        actor1_history.append(pos)

    def on_actor2_pose(msg: Pose) -> None:
        nonlocal actor2_xy, actor2_history
        pos = (float(msg.position.x), float(msg.position.y))
        actor2_xy = pos
        actor2_history.append(pos)
    ...
    try:
        while (time.time() - start) < duration_s:
            ...
            if now >= next_log:
                ...
                robot_heading = robot_heading_degrees(robot_pose) if robot_pose else float('nan')
                actor1_direction = calculate_direction_degrees(actor1_history)
                actor2_direction = calculate_direction_degrees(actor2_history)
                ...
                writer.writerow([
                    ...
                    int(goals_count),
                    safe_float(robot_heading),
                    safe_float(actor1_direction),
                    safe_float(actor2_direction),
                ])
                csv_file.flush()
