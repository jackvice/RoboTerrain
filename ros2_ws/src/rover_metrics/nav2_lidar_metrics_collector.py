...
class MetricsCollectorNode:
    ...
    def __init__(self, config: EnvironmentConfig):
        ...
        for actor_id, topic in config.actor_topics.items():
            self.node.create_subscription(Pose, topic, lambda msg, aid=actor_id: self._on_actor_pose(aid, msg), 10)

        self.goal_pub = self.node.create_publisher(PoseStamped, "/goal_pose", goal_qos)
        self.set_pose_client = self.node.create_client(SetEntityPose, f"/world/{config.world_name}/set_pose")
...
def main(env_name: str = "construct") -> None:
    ...
    next_log = start + 1.0
    step = 0
    duration_s = config.total_duration_min * 60
    last_clear_time = start
        ...
        while (time.time() - start) < duration_s:
            ...
            if collector.state.current_goal and collector.state.robot_xy:
                if is_goal_reached(collector.state.robot_xy, collector.state.current_goal, config.goal_threshold):
                    ...
                    collector.state = on_goal_reached(collector.state, new_goal)
