...
class BlockMetrics(NamedTuple):
    ...
    p08: float      # fraction of seconds with dmin < 0.8
    p12: float      # fraction of seconds with dmin < 1.2
...
METRICS: List[str] = ["p05", "p08", "p12", "goal_rate"]
...
def compute_block_metrics_array(
    ...
    Returns: np.ndarray of shape (n_blocks, 4) with columns [p05, p08, p12, goal_rate].
    ...
    for i in range(n_blocks):
        ...
        p08: float = float((block_dmin < 0.8).sum() / n_valid) if n_valid > 0 else 0.0
        p12: float = float((block_dmin < 1.2).sum() / n_valid) if n_valid > 0 else 0.0

        goals_in_block: float = float(block_goals.iloc[-1] - block_goals.iloc[0])
        goal_rate: float = goals_in_block / block_minutes

        results.append([p05, p08, p12, goal_rate])
