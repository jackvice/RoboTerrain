...
class RoverEnv(gym.Env):
    ...
    def get_target_info(self):
        ...
        if self.total_steps % 1000 == 0:  # Add debug prints
            print(f"Target heading (deg): {math.degrees(target_heading):.1f}")
            print(f"Current yaw (deg): {math.degrees(self.current_yaw):.1f}")
            print(f"Relative angle (deg): {math.degrees(relative_angle):.1f}")   
